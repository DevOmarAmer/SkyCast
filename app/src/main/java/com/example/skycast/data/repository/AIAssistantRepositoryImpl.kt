    ...
    override suspend fun getWeatherSummary(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val prompt = """
                    You are a smart and friendly local weather assistant. The current time is $currentTime.
                    ...
                    Write a short weather brief (max 3 sentences) in $langName, using a friendly conversational tone. 
                    Start with a suitable greeting based on the current time ($currentTime).
