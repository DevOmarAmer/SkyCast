    ...
    override suspend fun getWeatherSummary(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val greeting = if (language.lowercase().startsWith("ar")) {
                    when (hour) {
                        in 5..11 -> "صباح الخير"
                        in 12..16 -> "نهار سعيد"
                        in 17..21 -> "مساء الخير"
                ...
                val prompt = if (language.lowercase().startsWith("ar")) {
                    """
                        أنت خبير أرصاد جوية ودود ومساعد شخصي ذكي.
                        بناءً على بيانات الطقس الحالية:
                        - درجة الحرارة: $tempC درجة مئوية
                        - الحالة: $condition
                        - سرعة الرياح: $windSpeed م/إ
                        - توقعات المطر: ${if (isRainy) "نعم" else "لا"}
                        
                        اكتب ملخصًا قصيرًا للطقس (بحد أقصى 3 جمل) باللغة العربية، باستخدام نبرة محادثة ودودة.
                        ابدأ بـ "$greeting" واستخدم البيانات أعلاه لتقديم نصيحة حول ما يجب ارتداؤه وكيفية الاستعداد لبقية اليوم.
                        لا تستخدم تنسيقات معقدة؛ فقط نص عادي يسهل قراءته. أضف بضعة رموز تعبيرية مناسبة.
                    """.trimIndent()
                } else {
                    """
                        You are a smart and friendly local weather assistant. 
                        Based on the following current weather data:
                        - Temperature: $tempC °C
                        - Condition: $condition
                        - Wind Speed: $windSpeed m/s
                        - Rain Expected: ${if (isRainy) "Yes" else "No"}
                        
                        Write a short weather brief (max 3 sentences) in English, using a friendly conversational tone. 
                        Start your message with "$greeting".
                        Advise me on what to wear and how to prepare for the rest of the day. Do not use complex formatting (e.g. asterisks); just plain text that is easy to read.
